...
const getUserByEmail = (email) => {
    ...
    const insertUser = (email, lastname, firstname, bucketfolder) => {
        return new Promise((resolve, reject) => {
            ...
            dbConnection.query(sql, [email, lastname, firstname, bucketfolder], (err, results) => {
                if (err) {
                    reject(err);
                }
                else {
    ...
    const updateUser = (userid, lastname, firstname, bucketfolder) => {
        return new Promise((resolve, reject) => {
            const sql = "UPDATE users SET lastname = ?, firstname = ?, bucketfolder = ? WHERE userid = ?";
            dbConnection.query(sql, [lastname, firstname, bucketfolder, userid], (err, results) => {
                if (err) {
                    reject(err);
                }
                else {
